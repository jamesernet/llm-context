...
**The tracker copy is the source of truth** (trackers vary by repo). Whether a working copy is also committed is the repo's call: some keep one under `docs/briefs/`, and where they do, the tracker still wins on any disagreement — update the file. Others have retired that path and gitignore it, because a second copy only drifts; there, the brief IS the ticket and there is no file to save. Read `docs/agents/workflow.md` if `setup-skills` has been run here, else the repo's CLAUDE.md/AGENTS.md before creating one.
